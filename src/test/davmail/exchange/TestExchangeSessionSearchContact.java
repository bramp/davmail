...
public class TestExchangeSessionSearchContact extends AbstractExchangeSessionTestCase {
    public void testSearchPublicContacts() throws IOException {
        ...
        List<Contact> contacts = session.searchContacts(folderPath, ExchangeSession.CONTACT_ATTRIBUTES, null, 0);
        int count = 0;
    ...
    public void testSearchPublicContactsRange() throws IOException {
        String folderPath = Settings.getProperty("davmail.publicContactFolder");
        List<Contact> contacts = session.searchContacts(folderPath, ExchangeSession.CONTACT_ATTRIBUTES, null, 10);
        assertEquals(10, contacts.size());
    }

    public void testSearchPublicContactsWithPicture() throws IOException {
        String folderPath = Settings.getProperty("davmail.publicContactFolder");
        List<Contact> contacts = session.searchContacts(folderPath, ExchangeSession.CONTACT_ATTRIBUTES, session.isTrue("haspicture"), 0);
        int count = 0;
        for (Contact contact : contacts) {
    ...
    public void testSearchContacts() throws IOException {
        List<Contact> contacts = session.searchContacts(ExchangeSession.CONTACTS, ExchangeSession.CONTACT_ATTRIBUTES, null, 0);
        for (Contact contact : contacts) {
    ...
    public void testSearchContactsUidOnly() throws IOException {
        ...
        attributes.add("uid");
        List<Contact> contacts = session.searchContacts(ExchangeSession.CONTACTS, attributes, null, 0);
        for (Contact contact : contacts) {
    ...
    public void testGalFind() throws IOException {
        ...
        Map<String, Contact> contacts = session.galFind(session.startsWith("cn", "a"), null, 100);
        for (Contact contact : contacts.values()) {
            System.out.println(contact);
        }
        if (!contacts.isEmpty()) {
            Contact testContact = contacts.values().iterator().next();
